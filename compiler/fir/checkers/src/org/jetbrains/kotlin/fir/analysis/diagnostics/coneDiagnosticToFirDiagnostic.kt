...
private fun ConeDiagnostic.toFirDiagnostic(
    ...
    is ConeAmbiguityError -> when {
        this.applicability.isSuccess -> {
            FirErrors.OVERLOAD_RESOLUTION_AMBIGUITY.createOn(source, this.candidates.map { it.symbol })
        }
        this.applicability == CandidateApplicability.UNSAFE_CALL -> {
...
private fun mapInapplicableCandidateError(
    ...
    }.ifEmpty { listOfNotNull(FirErrors.INAPPLICABLE_CANDIDATE.createOn(source, diagnostic.candidate.symbol)) }
}
